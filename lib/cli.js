...
cli.responders.help = function () {
    var commands = {
        ...
        'stats': 'Get statistics on the underlying oeprating system and resource utilization',
        'list users': 'Show a list of all the resgistered (undeleted) users in the system',
    ...
    if (str) {
        ...
        uniqueInputs.some(function (input) {
            if (str.toLowerCase().indexOf(input) > -1) {
                matchFound = true;
                // Emit an event match the unique input, and include the full string given by the user
                e.emit(input, str);
                return true;
            }
        });
